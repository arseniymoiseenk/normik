...
def add_to_cart(salad_id):
    ...
    if 'salads' not in session:
        session['salads'] = []
    session['salads'].append({'name': salad.name, 'price': salad.price})
    session.modified = True
    return redirect(url_for('cart'))

@app.route('/cart')
def cart():
    salads = session.get('salads', [])
    return render_template('cart.html', salads=salads)
